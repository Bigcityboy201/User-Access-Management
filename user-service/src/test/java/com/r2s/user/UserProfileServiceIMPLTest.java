...
class UserProfileServiceIMPLTest {
	...
	// Simple helper to attach a spy logger if we ever want to verify logging
	// directly
	...
	void create_shouldSkipWhenUserAlreadyExists() {
		// ===== ARRANGE =====
		CreateUserProfileDTO dto = CreateUserProfileDTO.builder().username("existingUser").email("existing@example.com")
				.fullName("Existing User").roleNames(List.of("USER")).build();
	...
	void create_shouldHandleMissingRoles() {
		// ===== ARRANGE =====
		CreateUserProfileDTO dto = CreateUserProfileDTO.builder().username("newUser").email("new@example.com")
				.fullName("New User").roleNames(List.of("MISSING_ROLE")).build();

		when(userRepository.findByUsername("newUser")).thenReturn(Optional.empty());
		when(roleRepository.findByRoleName("MISSING_ROLE")).thenReturn(Optional.empty());

		User unsavedUser = User.builder().id(null).username("newUser").email("new@example.com").fullname("New User")
				.password("N/A").deleted(false).build();

		User savedUser = User.builder().id(100).username("newUser").email("new@example.com").fullname("New User")
				.password("N/A").deleted(false).build();
	...
	void create_shouldCreateUserWithoutRolesWhenNoRolesProvided() {
		// ===== ARRANGE =====
		CreateUserProfileDTO dto = CreateUserProfileDTO.builder().username("userNoRoles").email("noroles@example.com")
				.fullName("User No Roles").roleNames(null).build();

		when(userRepository.findByUsername("userNoRoles")).thenReturn(Optional.empty());

		User savedUser = User.builder().id(200).username("userNoRoles").email("noroles@example.com")
				.fullname("User No Roles").password("N/A").deleted(false).build();
	...
	void create_shouldHandleDatabaseException() {
		// ===== ARRANGE =====
		CreateUserProfileDTO dto = CreateUserProfileDTO.builder().username("dbErrorUser").email("dbError@example.com")
				.fullName("DB Error User").roleNames(List.of("USER")).build();
	...
	void create_shouldLogWarningWhenRoleNotFound() {
		// ===== ARRANGE =====
		CreateUserProfileDTO dto = CreateUserProfileDTO.builder().username("userMissingRole")
				.email("missingrole@example.com").fullName("User Missing Role")
				.roleNames(List.of("EXISTING_ROLE", "MISSING_ROLE")).build();
		...
		User savedUser = User.builder().id(300).username("userMissingRole").email("missingrole@example.com")
				.fullname("User Missing Role").password("N/A").deleted(false).roles(List.of(existingRole)).build();
	...
	void create_shouldAssignMultipleRoles() {
		// ===== ARRANGE =====
		CreateUserProfileDTO dto = CreateUserProfileDTO.builder().username("multiRoleUser").email("multi@example.com")
				.fullName("Multi Role User").roleNames(List.of("ROLE_USER", "ROLE_ADMIN")).build();
		...
		User savedUser = User.builder().id(400).username("multiRoleUser").email("multi@example.com")
				.fullname("Multi Role User").password("N/A").deleted(false).roles(List.of(userRole, adminRole)).build();
